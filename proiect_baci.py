...
def send_email(to_email, new_password):
    """
    Sends an email with the new password to the specified email address.
    Replace the email configuration with your actual SMTP server details.
    """
    sender_email = "faker_acc@yahoo.com"
    sender_password = "`1qaz@WSX"  # Use environment variables for sensitive data

    smtplib.SMTP('smtp.mail.yahoo.com', 587)

    subject = "Password Reset for Master Account"
    ...
    try:
        # Connect to Yahoo's SMTP server on port 587 (TLS)
        with smtplib.SMTP('smtp.mail.yahoo.com', 587) as server:
            server.set_debuglevel(1)  # Optional: To see debug information for troubleshooting
...
@app.route('/reset_password_master_account', methods=['POST', 'GET'])
def reset_password_master_account():
    if request.method == 'POST':
        try:
            # Parse request data
            ...
            master_account_email = data.get('master_account_email')
            if not master_account_email:
                return jsonify({"error": "master_account_email is required."}), 400
    
            # Establish connection to the database
            connection, cursor = set_connection_cursor()
    
            # Query to check if the account exists using the provided email
            check_query = """
                SELECT COUNT(*)
                FROM account_table
                WHERE master_account_email = %s
            """
            cursor.execute(check_query, (master_account_email,))
            existing_count = cursor.fetchone()[0]
            ...
            if existing_count == 0:
                ...
                cursor.close()
                connection.close()
                return jsonify({"error": "Master account with this email not found."}), 404
            ...
            update_query = """
                ...
                SET master_account_password = %s
                WHERE master_account_email = %s
            """
            cursor.execute(update_query, (hashed_password, master_account_email))
            connection.commit()

            # Send the new password to the email
            try:
                send_email(master_account_email, new_password)
            except Exception as e:
                cursor.close()
                connection.close()
        ...
        return render_template('reset_password_master_account.html')
